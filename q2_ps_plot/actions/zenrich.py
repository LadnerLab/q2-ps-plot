...
def zenrich(
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        if not exact_z_thresh:
            threshRange = list(reversed(range(lower_z_thresh, upper_z_thresh,
                                              step_z_thresh
                                              )))
        else:
        ...
        columnNms = ["Peptide", "sample", "sample_value",
        ...
        if peptide_metadata:
            ...
            enrichedDf = metaDf.merge(enrichedDf, how="right",
        ...
        #collect x axis data
        x = np.array([np.mean([float(negative_data[sn][pn])
                               for sn in negative_controls
        ...
        for sample in dropNames.keys():
            sNames = sample.split("~")
            y = np.array([np.mean([float(data[sn][pn])
                                   for sn in sNames
        ...
        sample_dropdown = alt.binding_select(options=dropList,
                                             name="Sample Select"
                                             )
        sample_select = alt.selection_single(fields=["sample"],
                                             bind=sample_dropdown,
                                             name="sample",
                                             init={"sample": dropList[0]}
                                             )
