...
def zscatter(
    ...
    #     highlight_df.to_csv("highlight.tsv", sep="\t", index=False)
    ...
    if spline_file:
        ...
        spline_dict = { "x": list(), "y": list(), "pair": list() }

        for pair in pairs:
            spline_dict["x"].append(spline_data_df.loc[:, pair[0]].to_list())
            spline_dict["y"].append(spline_data_df.loc[:, pair[1]].to_list())
            spline_dict["pair"].append(f"{pair[0]}~{pair[1]}")
        
        spline_df = pd.DataFrame(spline_dict)
        print(f"Spline DataFrame:\n{spline_df}")
